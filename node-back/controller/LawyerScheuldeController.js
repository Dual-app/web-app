  ...
  try {
    ...
    if (exists) {
      return res.status(400).json({
        message: "Schedule already exists for the same lawyer, date, and time.",
      });
    } else {
      // Find the current max Schedule_ID
      const lastSchedule = await LawyerSchedule.findOne().sort({
        Schedule_ID: -1,
      });
...
exports.getAllLawyerSchedules = async (req, res) => {
  try {
    const schedules = await LawyerSchedule.find();
...
exports.deleteLawyerSchedule = async (req, res) => {
  try {
    const scheduleId = Number(req.params.id);
    const deletedSchedule = await LawyerSchedule.findOneAndDelete({
      Schedule_ID: scheduleId,
    });
    if (!deletedSchedule) {
      return res.status(404).json({ message: "Lawyer Schedule not found" });
